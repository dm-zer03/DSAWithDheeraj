...
public class ArtiPoints {
    ...
    public static void dfs(ArrayList<Edge> graph[], int curr, int par, int dt[], int low[], int time, boolean vis[], boolean isAp[]) {
        ...
        for (int i = 0; i < graph[curr].size(); i++) {
            ...
            if (par == neigh) {
                ...
                if (par != -1 && dt[curr] <= low[neigh] && !isAp[curr]) {
                    System.out.println("Ap :" + curr);
                    isAp[curr] = true;  // Mark as articulation point
                }
                children++;
            }
        }

        if (par == -1 && children > 1 && !isAp[curr]) {
            System.out.println("Ap :" + curr);
            isAp[curr] = true;  // Mark as articulation point
        }
    }
    ...
    public static void getAp(ArrayList<Edge> graph[], int V) {
        ...
        boolean isAp[] = new boolean[V]; // To keep track of articulation points
